...
namespace base_tools {
    UDPSocket::UDPSocket(bool useConnect, std::function<void(int, const std::string&)> onError, int socketId) : BaseSocket(
            onError, UDP, socketId) {
        if (!useConnect) {
    ...
    void UDPSocket::SendTo(const std::string& message, const std::string& host, uint16_t port,
                           std::function<void(int, const std::string&)> onError) {
        this->SendTo(message.c_str(), message.length(), host, port, onError);
    }

    void UDPSocket::SendTo(const char *bytes, size_t byteslength, const std::string& host, uint16_t port,
                           std::function<void(int, const std::string&)> onError) {
    ...
    void UDPSocket::Connect(const std::string& host, uint16_t port, std::function<void(int, const std::string&)> onError) {
        struct addrinfo hints, *res, *it;
        memset(&hints, 0, sizeof(hints));
        hints.ai_family = AF_INET;
        hints.ai_socktype = SOCK_DGRAM;
    ...
    void UDPSocket::Connect(uint32_t ipv4, uint16_t port, std::function<void(int, const std::string&)> onError) {
        ...
        while (true) {
            ...
            if (messageLength < 0) {
                ...
                if (udpSocket->onMessageReceived)
                    udpSocket->onMessageReceived(std::string(tempBuffer).substr(0, messageLength),
    ...
    void UDPSocket::ReceiveFrom(UDPSocket *udpSocket) {
        ...
        while (true) {
            int messageLength;
            messageLength = recvfrom(udpSocket->sock, tempBuffer, udpSocket->BUFFER_SIZE, 0, (sockaddr *) &hostAddr,
                                     &hostAddrSize);
            ...
            if (messageLength < 0) {
                ...
                tempBuffer[messageLength] = '\0';
                if (udpSocket->onMessageReceived)
                    udpSocket->onMessageReceived(std::string(tempBuffer).substr(0, messageLength), ipToString(hostAddr),
                                                 ntohs(hostAddr.sin_port));

                if (udpSocket->onRawMessageReceived)
                    udpSocket->onRawMessageReceived(tempBuffer, messageLength, ipToString(hostAddr),
                                                    ntohs(hostAddr.sin_port));
            }
        }
    }
